...
export default function AdjustmentForm({ items, warehouses, suppliers }) {
  ...
  const [activeForms, setActiveForms] = useState("add");
  return (
    <div>
      {/* Header */}
      <FormHeader title="New Adjustment" href="/dashboard/inventory/adjustments" />

      <div className="border-b dark:border-gray-700 w-full max-w-4xl px-4 py-2 bg-white border border-gray-200 mx-auto my-4 shadow rounded">
          ...
          {tabs.map((tab, i) => {
            ...
            return (
              <li className="me-2" key={i}>
                <button
                  onClick={() => setActiveForms(tab.form)}
                  className={`${
                    activeForms === tab.form
